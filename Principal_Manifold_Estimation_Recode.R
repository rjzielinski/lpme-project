...
ker2 <- function(x, mu, sigma) {
  yseq <- dnorm((x - mu) / sigma)
  return((sigma ^ {-length(x)}) * prod(yseq))
}
# ker3 <- function(x, mu, sigma) {
#   yseq <- dnorm(t(t(x) - mu) / sigma)
#   return((sigma ^ {-length(x)}) * prod(yseq))
# }
...
weight.seq2 <- function(x.obs, mu, sigma, epsilon=0.001, max.iter=1000) { 
  ...
  for(j in 1:N){
    A.prepare=function(x){ return(ker2(x,mu[j,],sigma))  }
