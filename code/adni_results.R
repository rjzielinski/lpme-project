...
plot_ly(
  x = lthal_ex_lpme_part_recon[, 2],
  y = lthal_ex_lpme_part_recon[, 3],
  z = lthal_ex_lpme_part_recon[, 4],
  frame = lthal_ex_lpme_part_recon[, 1],
  type = "scatter3d",
  mode = "markers"
)
...
for (time_idx in seq_along(time_points)) {
  ...
  data_cloud <- pv$PolyData(temp_data)
  data_mesh <- estimate_mesh_volume(data_cloud, alpha_vals)
  lpme_cloud <- pv$PolyData(temp_lpme_part_recon)
  lpme_mesh <- estimate_mesh_volume(lpme_cloud, alpha_vals)
  pme_cloud <- pv$PolyData(temp_pme_part_recon)
  pme_mesh <- estimate_mesh_volume(pme_cloud, alpha_vals)
  pc_cloud <- pv$PolyData(temp_pc_part_recon)
  pc_mesh <- estimate_mesh_volume(pc_cloud, alpha_vals)
