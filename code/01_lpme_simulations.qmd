...
Also interesting is the fact that the distribution of the errors for the models fit to partitioned or augmented data
differ substantially, with the partitioned models having errors that are higher than the augmented models at all
quantiles, but particularly at the right tail. This pattern is particularly prevalent when comparing the PME results
between augmented and partitioned datasets, as the temporal smoothing in the LPME algorithm seems to address some of the
inconsistency in the volume estimates. 

I will need to further explore whether the cases showing high levels of error are
failing to fit at a limited number of time points, or whether the error is more pervasive. However, the fact that the
median root squared error is also substantially higher for the partitioned models indicates that the partitioning
approach is routinely underperforming the augmented models. This contradicts what I had originally expected, as I considered it more
likely that the augmentation process would be less consistent in removing self-intersections in the data, resulting in
higher errors.
