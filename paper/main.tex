...
\begin{itemize}
\item $d$ and $D$ are two positive integers denoting dimensions, and $d<D$; \meng{Add some explanations of the dimensionalities $d$ and $D$.}
...
A manifold $M_{f^{*}}^{d}$ determined by $f^{*}$ is called a principal manifold for $\boldsymbol{X}$ with the tuning parameter $\lambda$ if 
  ...
  \end{equation}
\end{definition}
\meng{Add some explanations for the terms in Eq.~\eqref{eq:5}.}
In this setting, we can see that $\pi_{f_{\lambda}^{*}}(\boldsymbol{X})$ maps the $D$-vector $\boldsymbol{X}$ to a $d$-dimensional parameterization, while $f_{\lambda}^{*}$ embeds the $d$-dimensional parameterization in the original $D$-dimensional space.
