...
\begin{equation}
  \mathcal{D}(\hat{f}_\lambda) = \frac{1}{I}\sum_{i=1}^{I}\|x_i - \hat{f}_\lambda(\pi_{\hat{f}_\lambda}(x_i))\|_{\mathbb{R}^{D}}^2 \label{eq:10}
\end{equation}
...
\begin{equation}
  \mathcal{L}(f_t) = (\mathbf{X_t} - E\mathbf{s}_t - R^{T}\mathbf{\alpha}_t)^{T}(\mathbf{X} - E\mathbf{s}_t - R^{T}\mathbf{\alpha}_t) + \gamma\mathbf{s}_t^{T}E\mathbf{s}_t, \label{eq:12}
\end{equation}
...
yielding function
\begin{equation}
  f_t^*(r) = \sum_{j=1}^{N}s_{jt}^* \eta_{4-d}\left(\|r - r_j^*\|\right) + \sum_{k=1}^{d + 1}\alpha_{kt}^*\phi(r)_k. \label{eq:14}
\end{equation}
...
\begin{equation}
  f_{\gamma}(t, \mathbf{r}) = \sum_{j=1}^{N}\mathbf{s}_{\gamma}(t)_j \eta_{4-d}\left(\|\mathbf{r} - \mathbf{r}_j^*\|\right) + \sum_{k=1}^{d+1}\mathbf{\alpha}_{\gamma}(t)_k \phi_k(\mathbf{r}). \label{eq:17}
\end{equation}
...
Note that alternative cross validation methods, such as $k$-fold cross validation, could easily be used in place of leave-one-out cross validation to better meet computational demands.
...
\For{t = 1, 2, \dots, T} {
  ...
  Let $N = \max(N_t)$, $\left\{r_i^*\right\}_{i=1}^{N}$ be a grid spanning the range of estimated values of $\left\{r_{i, j}\right\}_{i=1, t=1}^{N_t, T}$, and $X_{i, t} = f_t(r_{i}^*)$ for $i = 1, \dots, N$ and $t = 1, \dots, T$\;
