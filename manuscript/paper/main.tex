...
\if1\blind
{
  \title{\bf \revision{Noise-corrected hippocampal shape and volume trajectories in Alzheimer's disease using longitudinal principal manifold estimation}}
  \author{
...
Hereafter, we utilize the following notation to describe the LPME framework for generic low and high dimensions, $d$ and $D$.
\revision{Let $\left\{x_{it}\right\}_{i=1, t=1}^{I_t, T}$ represent the $I = \sum_{t=1}^{T}I_t$ observations in $\mathbb{R}^D$ (i.e., each $x_{it}\in\mathbb{R}^D$) for each participant, where $I_t$ denotes the total number of observations available within the image recorded at time point $t$, with $T$ total time points.} As in the setting described above, at each time point $t$, these observations lie in the vicinity of a $d$-dimensional manifold and are corrupted by $D$-dimensional noise. In this setting, the manifold to estimate is represented by the following, where $t$ coincides with the time dimension
...
\end{definition}
 ...
 By minimizing the functional $\mathcal{K}_{\lambda, \gamma, \mathbb{P}}$ in (\ref{eq:newKappa}), our goal is to (i) minimize the distance between each data point $\mathbf{X}_t$ and its projection $f_t\left(\pi_{f_t}(\mathbf{X}_t)\right)$ on the fitted manifold at each time point $t$, (ii) penalize the roughness of the fitted manifold at each time point $t$ depending on the value of the tuning parameter $\lambda_t$, and (iii) impose smoothness of the function $F$ over time regularized by the tuning parameter $\gamma$. These goals correspond to minimizing the first, second, and third additives of the functional $\mathcal{K}_{\lambda, \gamma, \mathbb{P}}$ in (\ref{eq:newKappa}), respectively. \revision{Note that this minimization problem builds on the penalized regression problem defined in Equation \ref{eq:pme_kappa}, where we now additionally penalize roughness in longitudinal changes in the estimated principal manifold.} \revision{By penalizing the spatial and longitudinal roughness in the underlying manifold, we effectively assume that the target manifold itself, as well as any changes in the manifold, must be able to be describable using smooth functions.} Using separate smoothing parameters provides additional flexibility in situations where a manifold with a high level of spatial roughness shows minimal changes over time, or vice versa. \revision{While this is a useful distinction in the neuroimaging context, in other settings, it may be valuable to consider mixed partial derivatives in space and time, an addition that may be pursued in future work.}
...
\begin{table}[ht]
  ...
  \resizebox{\textwidth}{!}{\begin{tabular}{|c c c c c|}
    ...
    \hline
    1 & 1 & 2 & $\left(r, \ \alpha \text{sin} \ (\beta r + \frac{\pi}{2})\right) + \iota$ & $-3 \leq r \leq 3$ \\
    2 & 1 & 2 & $\left(r, \ \alpha\text{sin} \ (\beta r)\right) + \iota$ & $-3\pi \leq r \leq 3\pi$ \\
    3 & 1 & 2 & $\left(\alpha_1 \text{cos} \ (\beta_1 r), \ \alpha_2\text{sin} \ (\beta_2 r)\right) + \iota$ & $-\frac{4\pi}{5} \leq r \leq \frac{\pi}{2}$ \\
    4 & 1 & 3 & $\left(r, (\alpha_1r + \beta_1)^2, \ (\alpha_2r + \beta_2)^3\right) + \iota$ & $-1 \leq r \leq 1$ \\
    5 & 1 & 3 & $\left(r, \ \alpha_1\text{cos} \ (\beta_1 r), \ \alpha_2\text{sin} \ (\beta_2 r) \right) + \iota$ & $0 \leq r \leq 3\pi$ \\
    6 & 2 & 3 & $\left(\beta_1r_1, \ \beta_2r_2, \ \alpha_1(\alpha_2\|\beta\mathbf{r}\|^2)\right) + \iota$ & $-1 \leq r_1, r_2 \leq 1$\\
    7 & 2 & 3 & $\left(\alpha_1\beta_1r_1\text{cos} \ (\alpha_1r_1), \ \alpha_2\beta_2r_1\text{sin} \ (\alpha_2r_1), \ r_2\right) + \iota$ & $0 \leq r_1 \leq 3\pi$; \ $-1 \leq r_2 \leq 1$\\
    8 & 2 & 3 & $\left(\alpha_1\text{sin}(\beta_1r_1)\text{cos}(\beta_2r_2), \ \alpha_1\text{sin}(\beta_1r_1)\text{sin}(\beta_2r_2), \ \alpha_1\text{cos}(\beta_1r_1)\right) + \iota$ & $0 \leq r_1 \leq \pi$; \ $0 \leq r_2 \leq 2\pi$\\
    \hline
  \end{tabular}}
...
\revision{A factorial design is used to run the simulation studies, with the factor levels set as follows: 1) $\iota, \upsilon_{\text{a}}, \upsilon_{\text{p}} \in \left\{0.05, 0.1, 0.25, 0.5\right\}$; 2) study duration: $\left\{1, 2, 5\right\}$; 3) interval between images: $\left\{0.25, 0.5\right\}$; 4) longitudinal change model: Constant, Linear, Quadratic; 5) $\zeta \in \left\{0, 0.05, 0.1, 0.25, 0.5\right\}$. The sample size for each time point is set to 1,000 observations. Each combination of factor levels is run once, resulting in each embedding map being simulated a total of 5,760 times. \revision{Note that simulation results with duration set to 1 year and between-visit intervals set to 0.5 years yield simulated datasets with only 3 visits, allowing assessment of performance of the proposed method with limited time points.} \if1\blind{All code required to reproduce the simulation results and associated Figures is provided at \href{https://github.com/rjzielinski/lpme}{\texttt{https://github.com/rjzielinski/lpme}}. }\fi Note that while discrete time points were used in these simulations, the spline model is also able to accommodate continuous time values, as seen in the ADNI application given in Section \ref{s:application}.}
...
\begin{table}[h]
  ...
  \begin{tabular}{|c c c c c|}
    ...
    \hline
    1 & 0.033 (0.03) & {\bf 0.024 (0.025); 228.7} & 0.030 (0.031); 994.1 & 0.034 (0.026); 2.2 \\
    2 & 0.095 (0.167) & {\bf 0.099 (0.155); 250.1} & 0.116 (0.173); 436.5 & 0.106 (0.147); 3.0 \\
    3 & 0.059 (0.058) & {\bf 0.041 (0.062); 274.8} & 0.049 (0.060); 551.1 & 0.048 (0.059); 1.9 \\
    4 & 0.033 (0.031) & {\bf 0.020 (0.023); 260.9} & 0.031 (0.031); 1406.0 & 0.029 (0.027); 1.2 \\
    5 & 0.173 (0.325) & 0.213 (0.336); 292.3 & 0.234 (0.368); 548.5 & {\bf 0.167 (0.316); 4.3} \\
    6 & 0.085 (0.056) & {\bf 0.072 (0.051); 287.5} & 0.082 (0.054); 1496.5 & 0.155 (0.073); 710.3 \\
    7 & 0.026 (0.033) & 0.048 (0.032); 364.3 & {\bf 0.043 (0.020); 105.38} & 0.043 (0.028); 992.8 \\
    8A & 0.063 (0.057) & {\bf 0.046 (0.044); 369.2} & 0.054 (0.038); 133.0 & N/A (N/A); N/A \\
    8P & 0.063 (0.057) & {\bf 0.053 (0.062); 331.55} & 0.063 (0.057); 1166.4 & 0.102 (0.027); 995.1 \\
    \hline
  \end{tabular}
  \caption{{\footnotesize Mean Squared Distance comparison of simulated data, longitudinal principal manifold estimation (LPME)-, principal manifold estimation (PME)-, and principal curve / surface-based estimates to true values, Median (IQR)\revision{; Run Time}. The lowest algorithm-specific median (IQR)\revision{; time} are highlighted in bold\revision{, and times are given in seconds. Cases 8A and 8P indicate simulation results for case 8 when the data augmentation and partitioning approaches were used, respectively. Results are unavailable for the principal surface algorithm in case 8A because it is restricted to cases where $d = 2$.}}}
...
While the LPME algorithm demonstrated strong performance in a number of settings, it is important to note the assumptions under which the method performs well. First, because Isomap and PME are used for initialization and time-specific fitting, the assumptions required by these algorithms are also needed for the LPME algorithm to perform well. In practice this limits the success of LPME to situations where Isomap performs well, which excludes nonconvex manifolds, manifolds that have holes, or manifolds that have too high a level of curvature. The use of alternative methods, such as Locally Linear Embedding \citep{roweis2000Nonlinear} or Laplacian Eigenmaps \citep{belkin2003laplacian}, to generate the initial parameterization may alleviate this concern. A second assumption is that the LPME method requires relative stability in the overall shape of the underlying manifold structure. Thus, while this algorithm is useful for separating meaningful changes in a manifold over time from inter-observation error, it may not perform well when the error between observations is too substantial, similar to any smoothing method. How restrictive this requirement is depends largely on the area of application. When used to estimate subcortical surfaces in a single individual as suggested here, this tends not to be overly restrictive, as changes in the shape and size of a subcortical structure, while clinically meaningful, tend to be relatively small in magnitude compared to their defining features. \revision{It may be useful to explore alterations to the criterion function given in Equation \ref{eq:newKappa} that enforce a different set of assumptions if, for example, stronger prior beliefs exist regarding the nature of changes in the target manifold.} \revision{While not considered in this manuscript, one interesting addition to the approach proposed here is the inclusion of the stronger assumption of monotonicity in the longitudinal changes experienced by the target manifold.} \revision{Because this method uses a smoothing spline function to estimate longitudinal change in the underlying manifold, it can be applied to cases where time points are observed at discrete intervals or are randomly sampled from a continuous time interval.} \revision{Additionally, the method is applicable when a limited number of time points are observed, as long as those time points sufficiently characterize the properties of the longitudinal change function.}
...
The approach described in this article is limited to the estimation of manifolds from observations taken from a single structure over time, but extension of this method to manifold estimation for groups of similar structures \revision{(for example, hippocampi from an entire study group, rather than one participant)} will be a valuable contribution to the literature allowing population-level comparisons of time-dependent trajectories of shapes. \revision{While in this manuscript we focused on individual-level brain region segmentation, future work may consider modeling the data at the population level to draw statistical inferences on group comparisons or other hypotheses of interest}. \revision{Additional work may also consider alternative observation-level dependencies. In higher resolution imaging studies, a higher degree of dependence may be assumed between the intensities of voxels in the same neighborhood. Thus, investigation of including spatially-dependent noise in the proposed LPME framework is an important future research direction}.
